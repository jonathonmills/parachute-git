...
require_once get_template_directory() . '/aviators/plugins/properties/widgets.php';
 ...
 */
function aviators_properties_custom_tags() {
    add_rewrite_rule("^" . __('properties', 'aviators'). "/(?!page)([^/]+)/([^/]+)/?",'index.php?post_type=property&location=$matches[1]&property=$matches[2]','top');
}
add_action('init','aviators_properties_custom_tags');

function aviators_properties_custom_post_link($post_link, $id = 0) {
    $post = get_post($id);
...
add_filter('post_type_link', 'aviators_properties_custom_post_link', 1, 3);
